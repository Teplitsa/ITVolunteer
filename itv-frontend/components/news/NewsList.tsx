      ...
      {items.map((item, key) => {
        return (
          <Fragment key={`news-list-item-key-${key}`}>
            ...
            {!!(key % 2) && <div className="news-list__separator" />}
          </Fragment>
        );
      })}
    </div>
  ...
  return (
    <article className="news-list-item">
      <div className="news-list-item__content">
        ...
        <h2 className="news-list-item__title" dangerouslySetInnerHTML={{ __html: item.title }} />

        <div className="news-list-item__date">
          {utils.getTheDate({ dateString: item.dateGmt, stringFormat: "dd.MM.yyyy" })}
        </div>
