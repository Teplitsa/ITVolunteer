...
const TaskListStats: React.FunctionComponent = (): ReactElement => {
  ...
  const setStatusStats = useStoreActions(
    actions => actions.components.taskListFilter.setStatusStats
  );
  ...
  useEffect(() => {
    const statusMatch = router.asPath.match(/\/tasks\/(publish|closed|in_work)\/?/);
    if (statusMatch) {
    ...
    <div className="stats">
      {["publish", "in_work", "closed"].map((status, index) => {
        return (
          <span
            className={
              (!_.get(optionCheck, "status") && status === "publish") ||
              _.get(optionCheck, "status") === status
                ? "active"
                : ""
            }
            key={`StatusFilterItem${index}`}
            onClick={e => {
              statusFilterClickHandler(e, status);
            }}
          >
