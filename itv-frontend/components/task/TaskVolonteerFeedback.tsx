...
const TaskVolonteerFeedback: React.FunctionComponent = (): ReactElement => {
  ...
  const { id: taskId, approvedDoer, doers } = useStoreState(
    (state) => state.components.task
  );
  const doersRequest = useStoreActions(
    (actions) => actions.components.task.doersRequest
  );

  useEffect(() => {
    isLoggedIn && doersRequest(taskId);
  }, [isLoggedIn, taskId, doersRequest]);
        ...
        <div className="sidebar-users-block responses">
          {isTaskAuthorLoggedIn && (
            <div className="choose-doer-explanation">
              У вас есть 3 дня на одобрение/отклонение кандидата. После
              прохождения 3-х дней мы снимаем баллы.
            </div>
          )}
