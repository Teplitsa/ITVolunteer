...
class IPGeoBase 
{
	...
	function getRecordByCityId($cityId) {
		rewind($this->fhandleCIDR);
		while($str = fgets($this->fhandleCIDR)) {			
			$arRecord = explode("\t", trim($str));
			if($arRecord[4] != '-' && $cityId == $arRecord[4]) {
				return array('range' => $arRecord[2], 'cc' => $arRecord[3]);
			}
		}
		return false;		
	}
	...
	function getCityById($id) {
		$cityResult = $this->getCityByIdx($id);
		
		$result = null;
		if($cityResult) {
			$result = $this->getRecordByCityId($id);
