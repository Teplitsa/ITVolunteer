...
try {
	...
	echo 'first_long_work_notif_dt=' . $first_notif_dt . "\n";
	$is_time_to_archive_long_work = false;
	if($first_notif_dt) {
	    ...
	    if($now > $first_notif_time_start) {
	        $is_time_to_archive_long_work = true;
	        echo "TIME to archive long work tasks...\n";
	        $itv_archiver->archive_tasks($action_key, $task_status, $task_archive_days, ItvLog::$ACTION_TASK_LONG_WORK_ARCHIVE, $is_check_doers);
	    }
	}
	if(!$is_time_to_archive_long_work) {
	    echo "NOT TIME to archive long work tasks\n";
	}
