...
class ItvSiteStats {
    ...
    public function refresh_users_role_stats($per_page = 100) {
        ...
        while ( ! $is_stop ) {
            ...
            $sql = "SELECT * FROM {$wpdb->prefix}users AS u LEFT JOIN {$wpdb->prefix}usermeta AS um ON um.user_id = u.ID AND um.meta_key = 'activation_code' WHERE um.meta_value = '' OR um.meta_value IS NULL ORDER BY ID ASC LIMIT $offset, $per_page";
