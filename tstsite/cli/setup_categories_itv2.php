...
try {
    ...
    Itv_Setup_Utils::link_posts_from_old_to_new_terms($terms, $tax, $post_type);
    ...
    foreach($sub_level_terms as $top_term_slug => $sub_terms) {
        ...
        foreach($sub_terms as $sub_term) {
            $sub_term['parent'] = $top_term->term_id;
        }

        Itv_Setup_Utils::setup_terms_data($sub_terms, $tax);
        Itv_Setup_Utils::link_posts_from_old_to_new_terms($sub_terms, $tax, $post_type);
    }

    Itv_Setup_Utils::delete_terms_beoynd_parents($tax, array_map(function($term_data) {
